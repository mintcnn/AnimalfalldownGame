...
public class MenuSubscene extends SubScene{
	...
	public MenuSubscene(int h, int w) {
		...
		BackgroundImage imageBackground = new BackgroundImage(
				new Image(ClassLoader.getSystemResource("image/graybg.png").toString(), this.getHeight(), this.getWidth(),
						false, true),
	...
	public void moveSubScene() {
		...
		if (isHidden) {
			System.out.println(
					-GameSetting.GAME_WIDTH - this.getWidth() + ((GameSetting.GAME_WIDTH - this.getWidth()) / 2));
			transition.setToX(
					-GameSetting.GAME_WIDTH - this.getWidth() + ((GameSetting.GAME_WIDTH - this.getWidth()) / 2));
			isHidden = false;
		} else {
